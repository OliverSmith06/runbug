...
if(isset($_POST['signupSubmit'])){
    ...
    if(!empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['email']) && !empty($_POST['phone']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])){
        ...
        if((!preg_match('~[0-9]~', $_POST['phone'])) || (strpos($title, " ") !== false)){
            $sessData['status']['type'] = 'error';
            $sessData['status']['msg'] = 'You cannot have letters in your phone number';
                }else{
                if(1 === preg_match('~[0-9]~', $_POST['first_name'])){
            $sessData['status']['type'] = 'error';
            $sessData['status']['msg'] = 'You cannot have numbers in your first name';
                }else{
        if(1 === preg_match('~[0-9]~', $_POST['last_name'])){
            $sessData['status']['type'] = 'error';
            $sessData['status']['msg'] = 'You cannot have numbers in your last name';  
        }else{
